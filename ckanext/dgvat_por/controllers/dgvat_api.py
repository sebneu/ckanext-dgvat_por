...
class DgvatApiController(ApiController):
    log.fatal('In DgvatApiController')
    _actions = {}
    ...
    def action(self, logic_function, ver=None):
        log.fatal("action %s" % logic_function)
    ...
    def list(self, ver=None, register=None, subregister=None, id=None):
        ...
        log.fatal('self._get_action_from_map(action_map, %s, %s)' % ( register, subregister))
        log.fatal("getaction %s " % action)
        if not action:
            return self._finish_bad_request(
